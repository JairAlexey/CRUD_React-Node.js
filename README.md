...
cd src
npm install
Configura la base de datos MongoDB. Asegúrate de tener una instancia local de MongoDB ejecutándose y actualiza la URI de conexión en el archivo src/db.js si es necesario:
...
Copiar código
npm run dev
Abre una nueva terminal y navega al directorio client para instalar las dependencias del frontend:
...
Dirígete a la página de registro para crear una cuenta nueva.
Después de registrarte, inicia sesión con las credenciales creadas.
Al iniciar sesión, serás redirigido a la página de productos, donde podrás gestionar tus productos.
...
Próximos pasos
Despliegue en la nube: Configurar el despliegue en un servicio como Heroku o Vercel.
Mejoras de UI/UX: Estilizar mejor las páginas y hacerlas más responsivas.
Validación de datos: Añadir validaciones de campos tanto en el frontend como en el backend.
Contribuciones
